...
print("Rectangle drawn successfully")`,
		...
		FormatPoint(Point{X: x + width, Y: y + height}))
}

// DrawCircle generates a Lua script to draw a circle.
...
// FillArea generates a Lua script to flood fill an area.
func (g *LuaGenerator) FillArea(layerName string, frameNumber int, x, y int, color Color) string {
	escapedName := EscapeString(layerName)
	return fmt.Sprintf(`local spr = app.activeSprite
if not spr then
	error("No active sprite")
